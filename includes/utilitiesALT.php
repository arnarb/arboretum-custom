            ...
            for ($i = 0; $i < $event->custom['venues']; $i++) {
                ...
                if($event->$dates_label) {
                    for($j = 0; $j < $event->$dates_label; $j++) {
                        ...
                        $date = date($format, strtotime($event->$date_label));
                    ...
                    if ($event->$end_label) {
                        if ($end_date === null || $end_date < $event->custom[$end_label]) {
                            ...
                            $startDate = new DateTime($event->custom[$start_label]);
                            // $date->modify('-1 month');
                            // $startDate[$location->ID] = $date->format('Y-m-d H:i:s');
                            $endDate = new DateTime($event->custom[$end_label]);
                            // $endDate->modify('-1 month');
                            // $endDate[$location->ID] = $date->format('Y-m-d H:i:s');
                            ...
                            // $context['reoccurring_dates'] = $dates;
                            foreach($dates as $event_date) {
                                if ($currentDate < $event_date && !$found) {
                                    
                                    $start_date = $event->custom[$start_label];
                                    $found = true;
                                }
                            }
                        }
                    } elseif ($start_date === null || $start_date > $event->custom[$start_label]) {
                        
                        if ($currentDate < $event->custom[$start_label]) {
                            $start_date = $event->custom[$start_label];
                        }
                    }                    
                }
