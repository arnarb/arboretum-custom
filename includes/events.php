      ...
      case 'event_date':
        ...
        if ($venues > 0) {
          ...
          foreach ($venues as $venue) {
            ...
            if ($venue['event_dates']) {
              foreach ($venue['event_dates'] as $event_date) {
                echo date("F d Y g:i a, l", strtotime($event_date['date'])) . '<br>';
              }
            } else {
              if ($venue['end_date']) {
                $begin = new DateTime($venue['start_date']);
                ...
                foreach ($period as $date) {
                  echo $date->format("F d Y g:i a, l") . '<br>';
                }
              } else {
                echo date("F d Y g:i a, l", strtotime($venue['start_date'])) . '<br>';// strtotime($venue['start_date']) . '<br>';
              }              
            }
